...
void Channel::kickClient(ClientData* client, ClientData* target, const std::string& reason) {
  if (isOperator(client)) {
    removeClient(target);
    std::string kickMsg =
        createCmdRespMsg(Server::servername_, ERR_USERNOTINCHANNEL,
                         "KICK " + ch_name_ + " " + target->getNickname() + " :" + reason);
...
void Channel::setTopic(ClientData* client, const std::string& topic) {
  if (isOperator(client)) {
    topic_ = topic;
    std::string topicMsg = createCmdRespMsg(Server::servername_, RPL_CHANNELMODEIS,
                                            "TOPIC " + ch_name_ + " :" + topic);
