...
class ClientData {  // 一応hostname,servernameも入れておく。
 private:
  ClientData();
  int socket_;
  std::string nickname_;  // 最大9文字だが、クライアントは将来的にプロトコルが進化する可能性を考慮して、より長い文字列を受け入れるべき->今回は20文字で
  std::string username_;
  std::string hostname_;  // 最大63文字。それ以上の場合、ホストアドレスを格納。->超えたら63字にリサイズ(9/8現在)
 ...
 public:
  ...
  bool isValidNickname(const std::string& param) const;
